...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        positions = {}
        for map in livingGhostPositionDistributions:
            mostLikelyPosition = map.keys()[0]
            highestProbability = map[mostLikelyPosition]
            for key in map.keys():
                possiblePosition = key
                possibleProbability = map[key]
                if (possibleProbability > highestProbability):
                    highestProbability = possibleProbability
                    mostLikelyPosition = possiblePosition
            positions[highestProbability] = mostLikelyPosition

        scariestGhostPosition = positions[max(positions.keys())]


        bestPosition = Actions.getSuccessor(gameState.getPacmanPosition(), legal[0])
        bestAction = legal[0]
        print max(positions.keys())
        highestDistance = self.distancer.getDistance(bestPosition, scariestGhostPosition)
        for action in legal:
            possiblePosition = Actions.getSuccessor(gameState.getPacmanPosition(), action)
            possibleDistance = self.distancer.getDistance(possiblePosition, scariestGhostPosition)
            if (possibleDistance < highestDistance):
                highestDistance = possibleDistance
                bestAction = action
        return bestAction
